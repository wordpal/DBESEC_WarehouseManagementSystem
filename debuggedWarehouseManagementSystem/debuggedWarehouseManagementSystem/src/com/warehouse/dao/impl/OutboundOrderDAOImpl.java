...
public class OutboundOrderDAOImpl implements OutboundOrderDAO {
    ...
    public boolean insert(OutboundOrder order) {
        ...
        try {
            ...
            String sql = "INSERT INTO outbound_order (order_code, product_id, location_id, quantity, operator) VALUES (?, ?, ?, ?, ?)";
    ...
    public boolean update(OutboundOrder order) {
        ...
        try {
            conn = DatabaseUtil.getConnection();
            String sql = "UPDATE outbound_order SET order_code=?, product_id=?, location_id=?, quantity=?, operator=? WHERE order_id=?";
            pstmt = conn.prepareStatement(sql);
            pstmt.setString(1, order.getOrderCode());
            pstmt.setInt(2, order.getProductId());
            pstmt.setInt(3, order.getLocationId());
    ...
    @Override
    public List<OutboundOrder> getByPageWithConditions(int pageNum, int pageSize, 
        ...
        try {
            ...
            // 添加outboundNo条件（对应order_code字段）
            if (outboundNo != null && !outboundNo.trim().isEmpty()) {
                sqlBuilder.append("AND oo.order_code LIKE ? ");
                params.add("%" + outboundNo + "%");
            }
            ...
            // 添加warehouseId条件（对应location_id字段）
            if (warehouseId != null) {
                sqlBuilder.append("AND oo.location_id = ? ");
    ...
    @Override
    public int getCountWithConditions(Integer productId, String outboundNo, 
                                     String operator, Integer warehouseId, 
                                     Date startDate, Date endDate) {
        Connection conn = null;
        PreparedStatement pstmt = null;
        ...
        try {
            ...
            // 添加outboundNo条件（对应order_code字段）
            if (outboundNo != null && !outboundNo.trim().isEmpty()) {
                sqlBuilder.append("AND order_code LIKE ? ");
                params.add("%" + outboundNo + "%");
            }
